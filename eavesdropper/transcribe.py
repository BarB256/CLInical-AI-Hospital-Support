...
def transcribe_buffer(
    ...
    # join all audio chunks into one array and ensure correct number format for Whisper
    audio_data = np.concatenate(buffer).astype(np.float32)

    if audio_mod.is_silent(audio_data):
        return

    # send audio to Whisper - language=None means auto-detect, fp16=False for compatibility
    result = model.transcribe(audio_data, language=None, fp16=False)
...
def run_transcription_loop(model, output_path: Path) -> None:
    ...
    # open the microphone and start pushing audio chunks into the queue
    stream = audio_mod.start_stream(audio_queue)
    ...
    try:
        while is_running:
            # pick up all available audio chunks from the queue into the buffer
            ...
            # if conditions are met, transcribe the buffer then clear it for next round
            if should_transcribe(is_pause_detected, is_buffer_full, bool(buffer)):
                transcribe_buffer(buffer, model, output_path)
                buffer.clear()

            time.sleep(0.05)
    # always runs on exit - turn off mic and transcribe any remaining audio
    finally:
        stream.stop()
        stream.close()
        if buffer:
