...
import sys #for priting microphone errors
...
def calculate_rms(audio_chunk: np.ndarray) -> float:
    ...
    return float(np.sqrt(np.mean(audio_chunk ** 2))) #returns the volume level of an audio chunk
...
def start_stream(audio_queue: queue.Queue) -> sd.InputStream: #opens the microphone and starts pushing audio chunks into the queue
    """
    Open the default microphone and push audio blocks into audio_queue.
    Returns the stream so the caller can close it later.
    """
    def _callback(indata: np.ndarray, frames: int, time_info, status): #sounddevice calls this automatically every 0.5 seconds with fresh audio
        if status:
            print(f"[audio] {status}", file=sys.stderr) #if the microphone has an issue, print the error to the terminal
        audio_queue.put(indata[:, 0].copy()) #take mono channel from audio, copy it safely, and place it on the queue

    block_size = int(SAMPLE_RATE * BLOCK_DURATION) #16000 * 0.5 = 8000 samples per chunk sent to the queue

    stream = sd.InputStream( #configure the microphone stream with settings whisper requires
